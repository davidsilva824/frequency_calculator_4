### best combinations that p0riveliege diminishing the difference between regulars and irregulars of the regular plurals
### it uses a maximum of four groups of man/men 
...
def score_combo(packs_subset):
    ...
    # PRIMARY: minimize Reg-PL vs Reg-SG mean difference across corpora
    reg_diff = [abs(means[3][c] - means[2][c]) for c in range(3)]
    primary = max(reg_diff)                 # worst column

    # SECONDARY: overall group balance (as before)
    gaps = [gap([means[g][c] for g in range(4)]) for c in range(3)]
    secondary_main = max(gaps)
    secondary_sum  = sum(gaps)

    return primary, secondary_main, secondary_sum, gaps, means
...
best = []  # (primary, secondary_main, secondary_sum, gaps, means, ids)
...
for ids in itertools.combinations(range(len(packs)), num_stimuli):
    ...
    primary, secondary_main, secondary_sum, gaps, means = score_combo([packs[i] for i in ids])
    rec = (primary, secondary_main, secondary_sum, gaps, means, ids)
    best.append(rec)
...
# ---- report ----
for rank, (obj_main, obj_mid, obj_sum, gaps, means, ids) in enumerate(best, 1):
